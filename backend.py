...
class DynamicBackend:
    ...
    def handle_subdomains(self, qname):
        ...
        _log("subdomain: %s" % subdomain)

        regex = r"(?:^|[^0-9])((?:(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)[\.-]){3}(?:25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?))"
        ip_search = re.search(regex, subdomain)
        if not ip_search:
            if _is_debug():
                _log('unable to find ip string in subdomain %s' % subdomain)
            self.handle_self(qname)
            return
        else:
            ip_address = ip_search.group(1).replace("-",".")
